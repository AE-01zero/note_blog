...
public class SharedKnowledgeBaseController {
    ...
    @SaCheckLogin
    public ResponseEntity<Flux<ServerSentEvent<String>>> chatWithKnowledgeBaseStream(
        ...
        if (!knowledgeBaseService.hasPermission(chatDTO.getKnowledgeBaseId(), userId)) {
            return noBufferSse(Flux.just(
                    "{\"type\":\"ai_error\",\"message\":\"无权访问该知识库\"}",
                    "{\"type\":\"done\",\"status\":\"error\"}"
            ).map(this::toSseEvent));
        }

        chatDTO.setKnowledgeBaseId(chatDTO.getKnowledgeBaseId());
        Flux<ServerSentEvent<String>> stream = consultantService.chatWithSharedKnowledgeBaseStructured(chatDTO, userId)
                .subscribeOn(Schedulers.fromExecutor(streamingExecutor))
                .timeout(Duration.ofSeconds(120), Flux.just(
                        "{\"type\":\"ai_error\",\"message\":\"共享知识库AI响应超时，请稍后重试\"}",
                ...
                .onErrorResume(throwable -> {
                    ...
                    return Flux.just(
                            "{\"type\":\"ai_error\",\"message\":\"共享知识库AI助手暂时不可用，请稍后重试\"}",
                            "{\"type\":\"done\",\"status\":\"error\"}"
                    ).map(this::toSseEvent);
                });
        return noBufferSse(stream);
    }
    ...
    private ResponseEntity<Flux<ServerSentEvent<String>>> noBufferSse(Flux<ServerSentEvent<String>> stream) {
